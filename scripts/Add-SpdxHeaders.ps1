<#
    ...
    SPDX license header and copyright notice to every OSS .cs source file under src/, tests/ and samples/
...
$files = Get-ChildItem -Path (Join-Path $root 'src'), (Join-Path $root 'tests'), (Join-Path $root 'samples') -Recurse -Filter *.cs |
    Where-Object { $_.FullName -notmatch '\\(bin|obj)\\' }

$changed = 0
foreach ($file in $files) {
    $content = [System.IO.File]::ReadAllText($file.FullName)
    if ($content.Substring(0, [Math]::Min(200, $content.Length)).Contains('// SPDX-License-Identifier:')) {
        continue
    }
