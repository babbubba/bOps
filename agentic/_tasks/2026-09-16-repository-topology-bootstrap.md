...
- [ ] Confirm private license/copyright holder and collaborator/team access.
...
- [ ] Add private license notice, README, security/contact policy, `.gitignore`, CODEOWNERS and
      branch protection suitable for proprietary code.
...
- [ ] Configure secret scanning and CI with no dependency on a developer machine.
...
- [ ] Add bootstrap documentation and a safe script for recursive clone/update, clean-state checks,
...
- [ ] Confirm an unauthorized identity cannot initialize `bOps.Commercial`.
- [x] Public-only clone/use of `bOps` remains unchanged.
- [ ] Both submodules build/test through root orchestration and independently.
