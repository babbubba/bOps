...
Next implementation batch: **V1.3-B — Docker management** (V1.3-A, bounded cross-platform system events, is implemented); V1.3 then completes the local senior-operator diagnostic surface through V1.3-L before V1.3-M entitlement/plugin lifecycle
...
| V1.3 | In progress | V1.3-A (`system.events`) is implemented. Ordered public batches A–L complete the senior-operator local diagnostic surface; M adds the neutral entitlement boundary and local plugin lifecycle. |
...
| Docker image/build/volume management | V1.3-B | Planned; existing start/stop/restart and image listing remain baseline, with typed verified extensions. |
