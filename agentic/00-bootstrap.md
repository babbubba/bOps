...
decides and executes. V1.1 (batches A–H) is complete, including the cross-platform integration and
release gate, and its preview release `v1.1.0-preview.2` is tagged. The active milestone is V1.2
multi-agent orchestration; its design is ADR-0030 and its sub-tasks A–M are listed in the V1.2 task (A–L are implemented; M is the integration and release gate). All gates are defined only by the
