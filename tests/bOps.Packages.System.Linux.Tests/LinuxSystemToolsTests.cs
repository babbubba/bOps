...
public sealed class LinuxSystemToolsTests
{
    ...
    [Fact]
    public void ToolProvider_ContributesExactlyTheNineteenSystemAndProcessTools()
    {
        var names = new LinuxSystemToolProvider().GetTools().Select(t => t.Manifest.Name).ToList();

        Assert.Equal(
            [
                "system.info", "system.time", "system.reboot_pending", "system.apps", "system.devices", "system.events", "system.updates", "system.cpu", "system.memory", "system.disk", "process.list",
