...
public sealed class WindowsIdentityNativeTests
{
    ...
    [Fact] public void DsRoleLayoutAndDomainControllerRoleValuesAreExact() { Assert.Equal(4, Marshal.OffsetOf<WindowsNetApi.DomainRoleInfo>(nameof(WindowsNetApi.DomainRoleInfo.Flags)).ToInt32()); Assert.Equal(8, Marshal.OffsetOf<WindowsNetApi.DomainRoleInfo>(nameof(WindowsNetApi.DomainRoleInfo.DomainNameFlat)).ToInt32()); Assert.False(WindowsNetApi.IsDomainControllerRole(3)); Assert.True(WindowsNetApi.IsDomainControllerRole(4)); Assert.True(WindowsNetApi.IsDomainControllerRole(5)); }
    [Fact] public void LocalGroupMembersUsesLevel3AndPointerSizedResumeHandle() { Assert.Equal(3, WindowsNetApi.MembersInfoLevel); var method = typeof(WindowsNetApi).GetMethod("NetLocalGroupGetMembers", BindingFlags.NonPublic | BindingFlags.Static)!; Assert.Equal(typeof(UIntPtr).MakeByRefType(), method.GetParameters()[7].ParameterType); Assert.Equal(IntPtr.Size, Marshal.SizeOf<UIntPtr>()); }
