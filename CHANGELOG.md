...
- V1.3-F filesystem troubleshooting: bounded, no-link `fs.grep` and `fs.tail`; cross-platform
  ...
  length/SHA-256. `fs.tail` decodes strictly (rejecting malformed UTF-8 instead of silently
  substituting it) and correctly handles a `maxBytes` window that starts mid multi-byte character.
  `fs.locks` and `fs.permissions` report `complete:false` whenever their diagnostic source failed,
  was restricted, or was truncated by a process/descriptor/row/entry cap — never a partial scan
  presented as trustworthy. Windows `fs.permissions` reads a directory's ACL via `DirectorySecurity`
  rather than the file API. `fs.mkdir` verification confirms the path is a directory, not merely
  that something exists there. See
  [`docs/filesystem-troubleshooting.md`](docs/filesystem-troubleshooting.md).
