...
public abstract class IdentityUsersToolBase(string platform) : ITool
{
    ...
    public async Task<ToolCallResult> ExecuteAsync(ToolArguments arguments, CancellationToken ct = default)
    {
        ...
        return ToolCallResult.Success(JsonSerializer.Serialize(new { users = rows.Take(limit), observedCount = rows.Length, truncated, complete = Complete }, IdentityJson.Options));
    }
}
...
public abstract class IdentityGroupsToolBase(string platform) : ITool
{
    ...
    public async Task<ToolCallResult> ExecuteAsync(ToolArguments arguments, CancellationToken ct = default)
    {
        ...
        return ToolCallResult.Success(JsonSerializer.Serialize(new { groups = rows.Take(limit), observedCount = rows.Length, truncated = rows.Length > limit, complete = Complete }, IdentityJson.Options));
    }
}
...
public abstract class IdentitySessionsToolBase(string platform) : ITool
{
    ...
    public async Task<ToolCallResult> ExecuteAsync(ToolArguments arguments, CancellationToken ct = default)
    {
        ...
        return ToolCallResult.Success(JsonSerializer.Serialize(new { sessions = rows.Take(limit), observedCount = rows.Length, truncated = rows.Length > limit, complete = Complete }, IdentityJson.Options));
    }
}
